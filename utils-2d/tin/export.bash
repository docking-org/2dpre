...
echo $output_base_dir $p $REQUIRED_UPLOAD

if ! [ -z "$REQUIRED_UPLOAD" ]; then
	for cat_requirement in $(echo $REQUIRED_UPLOAD | tr ',' ' '); do
		res=$(psql -p $p -d tin -U tinuser -c "select * from tin_meta where varname = 'upload_name'" | tail -n+2 | grep $cat_requirement)
		[ -z "$res" ] && echo "couldn't find export requirement in database!" && exit 1
	done
fi
