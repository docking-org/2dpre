...
upload_requ=${upload_requ-$latest_upload}
...
for machine in $(cat $BINDIR/common_files/machines.txt); do
	...
	export BINDIR
	sbatch --cpus-per-task=5 -w $machine -o $log_dir/%a.out -J z22_snex_${export_type} --array=1-$n_dbs%1 $BINDIR/export_job.bash $export_type $export_dest $latest_upload
done
