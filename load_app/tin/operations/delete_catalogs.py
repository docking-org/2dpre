...
def delete_catalogs(args):
    transaction_id = args.transaction_id
    
    diffs = "/nfs/exb/zinc22/upload_diffs/"

    n_partitions = get_partitions_count()
    
    host = Database.instance.host
    port = Database.instance.port
    path = diffs + transaction_id + "/"+ host + ':' + str(port) 

    if not os.path.exists(path):
        print("no diff found at", path)
        return False
    ...
    Database.instance.call_file('/mnt/nfs/home/xyz/btingle/bin/2dload.testing/psql/tin/delete_codes.sql', vars=args)
