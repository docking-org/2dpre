...
old_work=$(find /dev/shm/2dpre_$(whoami) -type d -mtime +180 | wc -l)
if [ $old_work -ge 1 ]; then
	synchronize_all_but_first "removing_old_work" "find /dev/shm/2dpre_$(whoami) -type d -mtime +180 | xargs rm -r"
fi
...
mkdir -p /dev/shm/2dpre_$(whoami)/${JOB_ID}_${TASK_ID}
pushd $BINPATH
BINPATH=$PWD
popd
cd /dev/shm/2dpre_$(whoami)/${JOB_ID}_${TASK_ID}
...
[ -z $SKIP_DELETE] && rm -r /dev/shm/2dpre/${JOB_ID}_${TASK_ID}
