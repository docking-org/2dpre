...
begin 
    ...
    create table catalog_substance_cat_t (sub_id_fk bigint, cat_content_fk bigint, grp_id int) partition by hash(cat_content_fk);
    ...
    execute(format('copy catalog_substance_cat_t(sub_id_fk, cat_content_fk, grp_id) from ''%s'' with delimiter as E''\t'' null as ''\N''', folder));
