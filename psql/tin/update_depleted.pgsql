	...
	create or replace procedure do_update(diff_destination text) as
		...
		begin
			...
			for i in 0..(n_partitions-1) loop
				...
				execute(format('copy (update catalog_content_p%s cc set depleted = false from (select supplier_code from supplier_update_src_p%s) cu where cc.supplier_code = cu.supplier_code) to ''%s'' delimiter '' ''', i, i, diff_fn));
