...
begin;
	...
	create or replace procedure init_grouping() as $$
	begin
	...
	create or replace function do_grouping(i int) returns boolean as $$
	declare
		--i int;
		j int;
		n int;
	begin
		...
		while true loop
			...
			-- for partition in gGRP:
			--    insert into gstats_t1 (select grp_id, count(grp_id) from partition group by (grp_id)
			...
			--    
			--    with grpstats_all as (select sub_id, cat_id, g.grp_id as grp_id, t0.grp_cnt as cnt_t0, t1.grp_cnt as cnt_t1 from gGRP_%i g join gstats_t0_%i t0 on t0.grp_id = g.grp_id join gstats_t1_%i on t1.grp_id = g.grp_id),
			--
			--    grp_next as (insert into gSUB(sub_id, cat_id, grp_id) (select sub_id, cat_id, grp_id from grpstats_all where cnt_t0 != cnt_t1) returning *)
			--    insert into gfinal(sub_id, cat_id, grp_id) (select sub_id, cat_id, grp_id from grpstats_all where cnt_t0 = cnt_t1)
	...
	create or replace procedure finalize_grouping() as $$
	begin
	...
	create or replace procedure do_grouping_iteration(transaction_id text) as $$
	declare
		success boolean;
	...
	call do_grouping_iteration(:'transaction_id');
commit;
