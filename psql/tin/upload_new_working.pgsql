...
begin;
	...
	/*
	create table temp_load_super_ids ( -- catalog ids that are considered a part of the same "super" catalog for this upload
		cat_id int
	);
	...
	create table temp_load_p4 ( -- deleted codes from stage 2, to be deleted in stage 4
		cat_content_id bigint,
		cat_id int
	...
	*/
	...
	create or replace function upload_catcontent_bypart(part int, transid text, diff_file_dest text) returns int as $$
		...
		begin
			...
			execute(format('insert into to_delete(code_id, cat_id) (select cc.cat_content_id, cc.cat_id_fk from (select supplier_code, cat_content_id, cat_id_fk from catalog_content_p%s where cat_id in (select cat_id from temp_load_super_ids)) cc left join temp_load_p2_p%s tl on cc.supplier_code = tl.code where tl.code is null', part, part));
			...
			execute(format('copy (delete from catalog_content_p%s where cat_content_id in (select code_id from to_delete) returning *) to ''%s/cat/%s.deleted''', part, diff_file_dest, part));
			...
			-- we will delete from catalog_id later, at stage 4
			...
			insert into temp_load_p4(code_id) (select code_id from to_delete);
	...
	create or replace function delete_catsub_bypart(part int, transid text, diff_file_dest text) returns int as $$
		begin
			execute(format('copy (delete from catalog_id_p%s where cat_content_id in (select code_id from temp_load_p4_p%s) returning *) to ''%s/catsubcat/%s.id.deleted''', part, part, diff_file_dest, part));
			execute(format('copy (delete from catalog_substance_cat_p%s where cat_content_fk in (select code_id from temp_load_p4_p%s) returning *) to ''%s/catsubcat/%s.deleted''', part, part, diff_file_dest, part));

			execute(format('drop table temp_load_p4_p%s', part));
	...
	create or replace function upload(stage int, part int, transid text, diff_file_dest text) returns int as $$
		declare maxpart int;
		begin
			select ivalue from meta where svalue = 'n_partitions' into maxpart;
			case
				when stage = 1 then
				...
				when stage = 3 then
					...
					if part = (maxpart - 1) then
						truncate table catalog_substance_cat;
						insert into catalog_substance_cat (select * from catalog_substance);
					end if;
				when stage = 4 then
					perform delete_catsub_bypart(part, transid, diff_file_dest);
					if part = (maxpart - 1) then
						truncate table catalog_substance;
						insert into catalog_substance (select * from catalog_substance_cat);
					end if;
				else
