create or replace function get_table_partitions (parent_table varchar) returns table (partition_name text) as $$
	begin
...
create or replace function find_duplicate_rows_substance() returns int as $$
	declare
		partition text;
		query text;
	begin
	create temporary table tranche_id_corrections_t (sub_id bigint, tranche_id_max smallint);
	create temporary table sub_dups_corrections_t (like sub_dups_corrections);
	for partition in select * from get_table_partitions('substance_t') loop
		...
		execute(query);
		insert into sub_dups_corrections (select * from sub_dups_corrections_t);
...
create or replace function find_duplicate_rows_catcontent() returns int as $$
	declare
		partition text;
		query text;
	begin
	create temporary table cat_dups_corrections_t (like cat_dups_corrections);
	for partition in select * from get_table_partitions('catalog_content_t') loop
		...
		execute(query);
		insert into cat_dups_corrections (select * from cat_dups_corrections_t);
...
create or replace function find_duplicate_rows_catsubstance() returns int as $$
	declare
		partition text;
		query text;
	begin
	create temporary table catsub_dups_corrections_t (cat_sub_itm_id bigint);
	for partition in select * from get_table_partitions('catalog_substance_t') loop
		...
		execute(query);
		insert into catsub_dups_corrections (select * from catsub_dups_corrections_t);
