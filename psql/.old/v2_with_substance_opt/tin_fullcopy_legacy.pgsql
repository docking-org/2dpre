...
-- we are assuming substance opt patch here, but might be a bad assumption
-- this script is a bit of a tricky case
select invalidate_index('substance_pkey');
select invalidate_index('smiles_hash_idx');
...
alter table substance alter column smiles type varchar using smiles::varchar;
